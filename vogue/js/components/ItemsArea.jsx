...
export default function ItemsArea({ catName }) {
  ...
  // 태그처리 구분 코드 생성함수
  const makeCode = (data) => {
    // console.log("배열?", Array.isArray(data));
    // 배열 데이터는 태그 구성이 다름
    // runway 카테고리만 다름
      ...
      <main className={"main-area ibx " + selData.경로}>
        ...
        <div className="cat-cont-area">
          ...
          <section className="pt2">
            <div className="cbx bgi bg1-1">{makeCode(selData.타이틀[0])}</div>
            <div className="cbx bgi bg1-2">{makeCode(selData.타이틀[1])}</div>
            <div className="cbx bgi bg1-3">{makeCode(selData.타이틀[2])}</div>
          </section>
          <section className="pt2">
            <div className="cbx bgi bg2-1">{makeCode(selData.타이틀[3])}</div>
            <div className="cbx bgi bg2-2">{makeCode(selData.타이틀[4])}</div>
            <div className="cbx bgi bg2-3">{makeCode(selData.타이틀[5])}</div>
          </section>
        </div>
      </main>
    </div>
